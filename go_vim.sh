...
#docker run -v $volume_root -v $volume_usr -v $volume_var -v $volume_etc \
docker run -v $volume_root -v $volume_usr -v $volume_etc -v $volume_var\
	...
	-u 1040:1030 --workdir $workdir/workspace $image bash -c "tail -f"
       	#--workdir $workdir --privileged $image bash -c "tail -f"
